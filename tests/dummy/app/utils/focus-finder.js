...
function refresh() {
  ...
  [].forEach.call(internal.config.container.querySelectorAll(FOCUSABLE_ELEMENTS), _registerElement);
...
function _addNodeFromMutationEvent(node) {
  if (node.nodeType === 1 && node.nodeName !== '#comment') {
    ...
    [].forEach.call(node.querySelectorAll(FOCUSABLE_ELEMENTS), _registerElement);
...
function _removeNodeFromMutationEvent(node) {
  if (node.nodeType === 1 && node.nodeName !== '#comment') {
    ...
    [].forEach.call(node.querySelectorAll(FOCUSABLE_ELEMENTS), _unregisterElement);
...
function _addNodeFromDomNodeAddedEvent(event) {
  if (event.target.nodeType === 1 && event.target.nodeName !== '#comment') {
    // Register Child Nodes
    [].forEach.call(event.target.querySelectorAll(FOCUSABLE_ELEMENTS), _registerElement);
...
function _removeNodeFromDomNodeAddedEvent(event) {
  if (event.target.nodeType === 1 && event.target.nodeName !== '#comment') {
    // Register Child Nodes
    [].forEach.call(event.target.querySelectorAll(FOCUSABLE_ELEMENTS), _unregisterElement);
